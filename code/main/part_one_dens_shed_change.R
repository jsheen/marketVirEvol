...
get_perc_nonzero_change <- function(lambda, phi) {
  ...
  diff_virs_R0_psi <- sapply(diff_virs_2, function(x) as.numeric(strsplit(x, ',')[[1]][5]))
  diff_virs_R02_psi <- sapply(diff_virs_2, function(x) as.numeric(strsplit(x, ',')[[1]][6]))
  constrain_R0_psi <- which(diff_virs_R0_psi <= max_R0_consider & diff_virs_R0_psi >= 1 & diff_virs_R02_psi <= max_R0_consider & diff_virs_R02_psi >= 1)
