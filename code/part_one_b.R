...
# 2) set function to get R0 in the model ---------------------------------------
      ...
      for (psi_clean in psi_clean_range) {
        for (m_m in m_m_range) {
          ...
          for (vir in virs) {
            ...
            R0 <- get_R0(beta=beta, N_m=N_m, p=p, epsilon=epsilon, sigma=sigma, nat_mort=nat_mort, m_m=m_m, gamma=gamma, mort=mort, psi=psi, both=T)
            R0s <- c(R0s, R0)
          }
