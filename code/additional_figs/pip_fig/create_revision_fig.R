...
temp_plot <- ggplot(melted, aes(y = invader_virulence, x = resident_virulence, fill = factor(value))) + geom_tile() +
  ...
  xlab('Resident virulence') + ylab('Invader virulence') + ggtitle('PIP, No cleaning') + scale_x_continuous(breaks = seq(0, 1000, by = 250))
temp_plot
plot_ls[[1]] <- temp_plot
...
temp_plot <- ggplot(melted, aes(y = invader_virulence, x = resident_virulence, fill = factor(value))) + geom_tile() +
  scale_fill_manual(values = c("white", "black"), na.value='gray') +theme(legend.position="none", axis.text=element_text(size=15),
                                                                          axis.title=element_text(size=20),
                                                                          plot.title=element_text(size=20,face="bold")) +
  xlab('Resident virulence') + ylab('Invader virulence') + ggtitle('PIP + erad. res., No cleaning') + scale_x_continuous(breaks = seq(0, 1000, by = 250))
temp_plot
plot_ls[[3]] <- temp_plot
