...
get_R0 <- function(beta, m_f, S_f, b, p, epsilon, sigma, nat_mort, m_m, gamma, mort, psi, both=T) {
  R0_num = ((m_f * S_f) / (m_m + nat_mort - b))^(1-p) * beta * (psi * sigma + epsilon * sigma * (m_m + gamma + mort + nat_mort))
  R0_denom = (sigma + nat_mort + m_m) * (psi) * (m_m + gamma + mort + nat_mort)
